...
const GITHUB_TOKEN = process.env.REACT_APP_DEEPAk;
  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      try {
        ...
        const userData = await fetchWithAuth('https://api.github.com/users/deepakcs2003');
        ...
        while (hasMore) {
          const reposData = await fetchWithAuth(
            `https://api.github.com/users/deepakcs2003/repos?per_page=100&page=${page}`
          );
        ...
        const contributedData = await fetchWithAuth(
          'https://api.github.com/search/issues?q=author:deepakcs2003+type:pr+is:merged&per_page=100'
        );
